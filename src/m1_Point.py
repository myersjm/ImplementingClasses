...
class Point(object):
    ...
    def move_to(self, x, y):
        before = Point(self.x, self.y)
        self.x = x
        self.y = y
        self.numberOfMoves = self.numberOfMoves + 1
        self.sumOfDistances = self.sumOfDistances + self.get_distance_from(before)
    ...
    def get_distance_from_start(self):
        startPoint = Point(self.x2, self.y2)
        totalDistance = self.get_distance_from(startPoint)
        return totalDistance

    def get_distance_traveled(self):
        return self.sumOfDistances
